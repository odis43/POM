...
var serviceAccount = require('../PrivateKeyFirebasePom/pomapp-b418d-firebase-adminsdk-oehcd-11f3ef20ce.json');
    ...
    .then(async function () {
      const configs = {
        user: {
          ...
          client_user_id: request.headers.uid,
        },
        client_name: 'POM',
        products: PLAID_PRODUCTS,
...
app.post('/api/exchange_public_token', function (request, response, next) {
  console.log('public exchange has been fetched');
  ...
  Promise.resolve()
    .then(async function () {
      ...
      docRef
        .update(AT)
        .then(() => {
          console.log('access token is pushed');
        })
        .catch(error => {
